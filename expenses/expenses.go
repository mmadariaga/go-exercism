...
func CategoryExpenses(in []Record, p DaysPeriod, c string) (float64, error) {
	...
	filterByCategory := ByCategory(c)
	recordsInCategory := Filter(in, filterByCategory)

	if len(recordsInCategory) == 0 {
		return 0, errors.New("unknown category " + c)
	}

	filterByDaysPeriod := ByDaysPeriod(p)
	records := Filter(recordsInCategory, filterByDaysPeriod)
